...
106 manifest tools plus a generated hub and one bundled Arcade game (108 distributable HTML
files). The source is in `tools/`; committed, self-contained output is in `dist/`.
...
## Current project state (v4.3.5, 2026-09-07)

- **v4.3.5 is the current release package (2026-09-07).** It adds Optical Transfer Beta Test 1 as
  the 106th manifest tool and fourth Beta Tools card. The separate page packages the selected H66-R2
  carrier: V37/2,563-byte QR frames, ECC L mask 4, 30 presentations/s, sender precompute ring depth
  3, four receiver workers, processor capture, calibrated fixed ROI, ZXing fast global histogram,
  adaptive H40 residual recovery, and post-SHA lifecycle closure. Stable `optical.html`, existing
  `optical-beta.html`, their built artifacts, focused tests, and stable documentation remain
  byte-for-byte unchanged. It also bundles Mushroom Death Garden as a local, offline-capable eighth
  Arcade game. The release contains 106 tools plus the hub and bundled game (108 distributable HTML
  files), precaches 112 entries, and wires the Arcade and Optical Transfer focused gates into Pages
  CI. Release evidence lives under `tests/evidence/v4.3.5-release/`.
- **v4.3.4 was released and deployed on 2026-08-09.** It adds Optical Transfer Beta as the 105th
...
- V4 adds suite-wide favorites and recently-used quick access (core chrome + hub sections), a
  ...
  29-resource National Parks Explorer with a drawn boundary map, The Arcade (eight playable games,
  including bundled Mushroom Death Garden), and 26 new offline/keyless tools.
...
- Current release evidence and the final checklist live under `tests/evidence/v4.3.5-release/`;
...
- The headed Chromium gate verifies a real `beforeinstallprompt` event, zero manifest and
  ...
  `suite-v4-` precache. Full build, PWA, update, and 108-page smoke gates remain mandatory for
  future releases.
