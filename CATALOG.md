...
File → speaker → air → microphone → verified file download. The production page keeps manifest/final control on audible C0 BPSK OFDM and sends DATA on wider audible R1 QPSK OFDM at the observed 44.1 or 48 kHz AudioContext rate. Both use pilots/training, K=7 rate-1/2 convolutional FEC with soft Viterbi decode, interleaving, duplicated protected headers, and CRC32C. File data is split into 1,024-byte source blocks and sent as a finite deterministic robust-soliton/XOR fountain with about 35% equation overhead. The bounded incremental receiver reconstructs arbitrary binary bytes within the declared 1 MiB limit and creates no download until the repeated final identity and Web Crypto SHA-256 both agree.
...
- **Permission:** microphone on Receive only. Sender emits an intentionally audible 1.2–7.2 kHz modem signal with a conservative waveform level and visible warning. Hosted HTTPS/PWA is the supported mobile receive path.
- **Limits/defaults:** 1 byte–1 MiB files; 1,024-byte source blocks; finite robust-soliton fountain; C0 BPSK control and R1 QPSK DATA; observed 44,100/48,000 Hz contexts; four-second physical-frame ceiling.
