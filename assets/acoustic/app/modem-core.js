...
(function (root) {
  ...
  const LIMITS = Object.freeze({
    ...
    minFileBytes: 1, maxFileBytes: 16 * 1024 * 1024,
    parserFileCeiling: 64 * 1024 * 1024, minChunkBytes: 256,
    ...
    maxCandidates: 4, maxCandidateOffsets: 4097, maxPcmBlockSamples: 4096,
    maxWorkerQueue: 16, maxLogRecords: 2000, maxLogBytes: 1024 * 1024,
    maxShaBytes: 64 * 1024 * 1024,
  });
...
(function(root){
  ...
  const registry=Object.freeze({
    ...
    16:Object.freeze({id:16,name:"R1",fLo:1200,fHi:7200,modulation:"BPSK",fecId:C.FECS.K7_R12,cp:128,enabled:true}),
  });
  function plan(profileId,sampleRate){const id=typeof profileId==="string"?C.PROFILES[profileId]:profileId,profile=registry[id];if(!profile||!profile.enabled)return fail("BAD_PROFILE","disabled or unknown");if(sampleRate!==44100&&sampleRate!==48000)return fail("BAD_SAMPLE_RATE","grid must be 44100 or 48000");const n=512,deltaF=sampleRate/n,kLo=Math.ceil(profile.fLo/deltaF),kHi=Math.floor(Math.min(profile.fHi,.45*sampleRate)/deltaF),usable=[];for(let bin=kLo+2;bin<=kHi-2;bin++)usable.push(bin);const pilots=usable.filter((_,i)=>i%8===0),pilotSet=new Set(pilots),data=usable.filter(bin=>!pilotSet.has(bin)),symbolSamples=n+profile.cp;return ok(Object.freeze({...profile,sampleRate,n,deltaF,kLo,kHi,usable:Object.freeze(usable),pilots:Object.freeze(pilots),data:Object.freeze(data),symbolSamples,symbolSeconds:symbolSamples/sampleRate,rawBitRate:data.length/(symbolSamples/sampleRate),codedInformationRate:data.length/2/(symbolSamples/sampleRate)}));}
...
(function(root){
  ...
  function hermitian(plan,positive){const re=new Float64Array(plan.n),im=new Float64Array(plan.n);for(const [bin,value] of positive){if(!Number.isInteger(bin)||bin<=0||bin>=plan.n/2||!Number.isFinite(value))return fail("BAD_OFDM","frequency value");re[bin]=value;}for(let bin=1;bin<plan.n/2;bin++){re[plan.n-bin]=re[bin];im[plan.n-bin]=-im[bin];}return ok({re,im});}
...
(function (root) {
  ...
  function validateHeaderFields(frame,payload,forDecode){
    ...
    if(!Number.isInteger(frame.profileId)||(frame.profileId!==C.PROFILES.C0&&frame.profileId!==C.PROFILES.R1)||(frame.type!==T.DATA&&frame.profileId!==C.PROFILES.C0))return fail("BAD_PROFILE","profile");if(frame.fecId!==C.FECS.K7_R12)return fail("BAD_FEC","only K7_R12 enabled");
...
(function(root){
  ...
  const RAMP_SECONDS=.005,TARGET_RMS=.08,PEAK_LIMIT=.5,MAX_CFO_HZ=100;
  ...
  function frequencySymbol(plan,values,sequence){
    if(!(values instanceof Float32Array)||values.length!==plan.data.length)return fail("BAD_OFDM","data carriers");const positive=[];for(let i=0;i<plan.data.length;i++){if(!Number.isFinite(values[i])||Math.abs(values[i])>1)return fail("BAD_OFDM","data value");positive.push([plan.data[i],values[i]]);}for(const bin of plan.pilots)positive.push([bin,A.Preamble.sign(0x50494c4f,sequence,bin)]);return A.Preamble.hermitian(plan,positive);
  }
  ...
  function symbolParts(bits,plan,startingSequence,symbolCount){const parts=[];let offset=0;for(let symbol=0;symbol<symbolCount;symbol++){const values=new Float32Array(plan.data.length);for(let i=0;i<values.length;i++)values[i]=offset<bits.length?(bits[offset++]?-1:1):1;const frequency=frequencySymbol(plan,values,startingSequence+symbol);if(!frequency.ok)return frequency;const time=A.Fft.inverseReal(frequency.value.re,frequency.value.im);if(!time.ok)return time;parts.push(withPrefix(time.value,plan.cp));}return ok(parts);}
  function frameLayout(frameBytes,sampleRate,profileId){
    ...
    const headerCodeBits=(L.headerBytes*8+6)*2,bodyBytes=frameBytes.length-L.headerBytes,bodyCodeBits=(bodyBytes*8+6)*2,headerSymbols=Math.ceil(headerCodeBits/c0.value.data.length),bodySymbols=Math.max(L.minPayloadSymbols,Math.ceil(bodyCodeBits/body.value.data.length));
  ...
  function demap(samples,start,symbolCount,plan,channel,sequence,exactBits,cfo,tracker,metrics){if(symbolCount<0||symbolCount>L.maxPayloadSymbols||exactBits>symbolCount*plan.data.length)return fail("LIMIT_RESOURCE","demap dimensions");const soft=new Float32Array(exactBits);let at=0;for(let symbol=0;symbol<symbolCount;symbol++){const fft=readFft(samples,start+symbol*plan.symbolSamples,plan,cfo,tracker.phase);if(!fft.ok)return fft;const fit=phaseFit(fft.value,channel,plan,sequence+symbol);if(!fit.ok)return fit;metrics.phaseResidual.push(fit.value.residualRms);metrics.phaseSlope.push(fit.value.slope);const timing=-fit.value.slope*plan.n/(2*Math.PI);if(Math.abs(timing)>=.05){const updated=tracker.update(timing,.1);if(!updated.ok)return updated;}for(const bin of plan.data){if(at>=exactBits)break;const value=equalize(fft.value.re[bin],fft.value.im[bin],channel,bin),phase=fit.value.intercept+fit.value.slope*bin,decision=value.re*Math.cos(phase)+value.im*Math.sin(phase),normalized=value.confidence<.01?0:decision/Math.sqrt(Math.max(1e-6,value.variance));soft[at++]=Math.max(-127,Math.min(127,normalized*4));}}return ok(soft);}
  ...
  class StreamReceiver{
    constructor(config){if(!config||(config.sampleRate!==44100&&config.sampleRate!==48000)||(config.profileId!==1&&config.profileId!==16))throw new RangeError("invalid stream receiver config");this.sampleRate=config.sampleRate;this.profileId=config.profileId;this.capacity=this.sampleRate*L.maxPhysicalFrameSeconds+L.maxCandidateOffsets;this.buffer=new Float32Array(this.capacity);this.length=0;this.expectedAbsolute=null;this.metrics={blocks:0,queueHighWater:0,overflows:0,discontinuities:0,duplicates:0,outOfOrder:0,frames:0,decodeFailures:0,candidateRateLimited:0};}
    ...
    push(block,absoluteFrame,discontinuities=0){if(!(block instanceof Float32Array)||block.length<1||block.length>L.maxPcmBlockSamples||!Number.isSafeInteger(absoluteFrame)||absoluteFrame<0||!Number.isInteger(discontinuities)||discontinuities<0)return [Object.freeze({kind:"METRIC",code:"BAD_RX_BLOCK"})];for(const sample of block)if(!Number.isFinite(sample)||Math.abs(sample)>4)return [Object.freeze({kind:"METRIC",code:"BAD_SAMPLE"})];const events=[];if(discontinuities){this.reset(true);events.push(Object.freeze({kind:"METRIC",code:"DISCONTINUITY",count:discontinuities}));}if(this.expectedAbsolute!==null&&absoluteFrame!==this.expectedAbsolute){if(absoluteFrame<this.expectedAbsolute){this.metrics.outOfOrder++;events.push(Object.freeze({kind:"METRIC",code:"OUT_OF_ORDER"}));return events;}this.reset(true);events.push(Object.freeze({kind:"METRIC",code:"FRAME_GAP"}));}this.expectedAbsolute=absoluteFrame+block.length;this.metrics.blocks++;if(block.length>this.capacity-this.length){this.metrics.overflows++;this.reset(true);events.push(Object.freeze({kind:"METRIC",code:"OVERFLOW"}));return events;}this.buffer.set(block,this.length);this.length+=block.length;this.metrics.queueHighWater=Math.max(this.metrics.queueHighWater,this.length);
      const minimum=Math.ceil(this.sampleRate*RAMP_SECONDS)+2*512+2*640+2*26*640+8*640+64;let decodeAttempts=0;while(this.length>0&&decodeAttempts<4){decodeAttempts++;if(this.length<minimum)break;const result=decodeBurst(this.buffer.subarray(0,this.length),this.sampleRate,this.profileId);if(result.ok){const consumed=result.value.consumedSamples;events.push(Object.freeze({kind:"RX_FRAME",frame:result.value.frame,frameBytes:result.value.frameBytes,metrics:result.value.metrics}));this.buffer.copyWithin(0,consumed,this.length);this.length-=consumed;this.metrics.frames++;continue;}if(result.code==="TRUNCATED")break;this.metrics.decodeFailures++;const discard=Math.min(64,this.length);this.buffer.copyWithin(0,discard,this.length);this.length-=discard;if(this.metrics.decodeFailures%256===1)events.push(Object.freeze({kind:"METRIC",code:result.code}));if(this.length<minimum)break;}if(this.length>=minimum&&decodeAttempts>=4)this.metrics.candidateRateLimited++;return events;}
    snapshotMetrics(){return Object.freeze({...this.metrics,bufferedSamples:this.length,capacitySamples:this.capacity});}
  }
