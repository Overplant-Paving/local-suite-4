/* Optical Transfer deterministic and browser integration gate.
   Run: node tests/optical-built.mjs */
...
const URL = pathToFileURL(join(ROOT, "dist", "optical.html")).href;
const failures = [];
...

{
  ...
  await p.addInitScript(() => {
    ...
    window.__opticalLifecycle = { created: 0, terminated: 0, stopped: 0 };
...
const html = readFileSync(join(ROOT, "dist", "optical.html"), "utf8");
...
check("PWA app-shell cache includes optical.html", serviceWorker.includes('"optical.html"'));
